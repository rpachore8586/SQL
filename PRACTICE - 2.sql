...
--     Return employee_id, first_name, last_name, email, phone_number, hire_date, job_id, salary,commission_pct, manager_id 
--     and department_id.
...
-- employees whose first name does not contain the letter ‘M’. -- Sort the result-set in ascending order by department ID. 
...
-- From the employees table, write a SQL query to retrieve all employees who either :
...
-- write a sql query to return the average salary of each department. Return department_id, average_salary.
...
-- ** HAVING CLAUSE is use after group  by

-- From the following table, write a SQL query to find employees 
...
### HOMEWORK - 29.01.2026
use hr;
/*1. From the following table, write a SQL query to
find those employees whose salarie
are less than 6000. Return full name (first and last
name), and salary.*/
...
to find those employees whose
salary is higher than 8000. Return first name,
last name and department number and salary.*/

Select * from Employees;

SELECT 
    CONCAT(First_Name, ' ', Last_Name) AS Full_Name,department_id, Salary
FROM
    Employees
WHERE
    salary > 8000;
...
/*3. From the following table, write a SQL query to
find those employees whose
last name is "McEwen". Return first name, last
...
identify employees who do
not have a department number. Returnemployee_id,
first_name, last_name, email,
phone_number, hire_date, job_id,
salary,commission_pct, manager_id and
department_id.*/
...
/*5. From the following table, write a SQL query to
find the details of 'Marketing'
...
SELECT CONCAT(First_Name, ' ', Last_Name) AS Full_Name,hire_date,department_id, Salary
FROM Employees where First_Name not like '%m%'
order by department_id asc;

/*7. From the employees table, write a SQL query to
retrieve all
employees who either :earn a salary between 8000 and
12000
(inclusive) and receive a commission (commission_pct
is not null),
or were hired before '2003-06-05' and do not belong
to the departments with
IDs 40, 120, or 70. Return all fields.*/


use HR;
Select * from Employees
where salary between 8000 and 12000 and commission_pct is NOt null
or hire_date > '2003-06-05' and department_id not in (40,120 or 70);
...
find those employees who do not
earn any commission. Return full name (first and last
name), and salary.*/
SELECT
CONCAT(first_name, ' ', last_name) AS full_name,
salary
...
find the employees whose salary
is in the range 9000,17000 (Begin and end values are
included).
Return full name, contact details and salary.*/
SELECT
CONCAT(first_name, ' ', last_name) AS full_name,
CONCAT(email, ' ', phone_number) AS
contact_details,
salary
FROM
employees
...
first name ends with the letter ‘m’. Return the first
...
/*11. From the following table, write a SQL query to
find those employees whose salaries
are not between 7000 and 15000 (Begin and end values
are included).Sort the result-set
in ascending order by the full name (first and last).
...
find those employees who were
hired between November 5th, 2007 and July 5th, 2009.
Return full name (first and last),
job_id and hire date.*/
...
find those employees who work either
in department 70 or 90. Return full name (first and
last name), department id.*/

Select concat(First_Name,' ',Last_Name) as Full_Name,Department_id from employees
where department_id = 70 or 90;
...
employees
WHERE
department_id = 70 OR 90;

/*14. From the following table, write a SQL query to
find those employees who work under
a manager. Return full name (first and last name),
...
find the employees whose managers
hold the ID 120, 103, or 145. Return first name,
last name, email,
salary and manager ID.*/
SELECT
...
first names contain the letters D, S, or N. Sort the
result-set in descending
...
/*18. From the following table, write a SQL query to
find those employees who earn
aabove 11000 or the seventh character in their phone
number is 3. Sort the result-set
in descending order by first name. Return full name
(first name and last name),
hire date, commission percentage, email, and
...
/*19. From the following table, write a SQL query to
find those employees whose
first name contains a character 's' in the third
position. Return first name,
...
WHERE
first_name LIKE '__s%';
/*20. From the following table, write a SQL query to
find those employees work in
the departments that are not part of the department
50 or 30 or 80.
...
/*21. From the following table, write a SQL query to
find the employees whose
department numbers are included in 30, 40, or 90.
Return employee id, first name,
job id, department id.*/


SELECT 
    First_Name, Employee_ID, Job_Id, Department_id
FROM
    Employees
WHERE
...
Select Manager_id,count(*) as No_of_EMployees,sum(salary) as Total_Salary from employees
group by Manager_id
having Manager_id is Not Null
order by No_of_EMployees desc limit 5;
...
/*From the following table, write a SQL query to calculate the average salary of employees 
...
/*From the following table, write a SQL query to find those job titles 
...
from employees
where first_name like "D%" or
last_name like "d%";

/*From the following table, write a SQL query to find details of those jobs 
...
-- write a SQL query to find those employees who worked more than two jobs in the past. Return employee id.
...
-- From the following table, write a SQL query to find each job ids 
...
-- An ER model is define in SQL using the create table and Alter Table statement to establish 
-- table for entities and define relationship through primary keys & Foreign Keys
...
-- From the following tables, write a SQL query to calculate the difference between the maximum salary of 
-- the job and the employee's salary. Return job title, employee name, and salary difference
 
select concat(e.first_name,' ',e.last_name) as Full_Name,j.job_title,J.Max_Salary-j.Min_Salary as Salary_Diff
...
-- country, employees , locations and departments

select c.country_name ,l.city , count(d.department_id) as number_of_departments

from countries c join locations l using (country_id) join departments d using (location_id)

where d.department_id in (select department_id from employees group by department_id having count(*)>=2)

group by c.country_name , l.city;
 
 
 -- Windows Function in MySQL - It Performs a Calculations across a set of Table rows related to the current row,without merging those rows into single results.
 
-- Types of Windows Function
 -- Aggregate- Avg,Max,Min,Sum,Count ( Already Learn)
 -- Ranking - Row_No,Rank,Dense_Rank,Percent_Rank,Ntile
 -- Value - Lag,Lead,First Value,Last Value,Nth values
 
 -- Ranking
 
--  Example

-- Salary		Row_No  rank   Dense_Rank   Percent_Rank
-- 10000          1      1         1
-- 10000          2      1         1    
-- 10000          3      1         1    
-- 8000           4      4         2 
...
-- Row_No- Asign Unique,Sequnce No to  each row (1,2,3,4)
-- Rank - Asign Same rank to tied,same values, but skipped subsequent ranking (1,2,2,4)
-- Dense Rank - Assign the same rank to tied values without skipping subsequent ranking (1,2,2,3)
-- Percent Rank - Calculate relative percentile rank of row,returning value between 0 and 1,computed as (rank-1)/(Total Rows -1)
-- Ntile - It is wnidows function devides the order rows of a results set or partition in to specifieed numbe 
	      -- of approximately equal group (Buckets) and assign curresponding bucket number(starting from 1) to each row.


-- Value Function
--   Salary       Lead       Lag         First Value          Last Value
--   18000       17000      Null           18000                15000
--   17000       16000      18000          18000                15000
--   16000       15000      17000          18000                15000 
--   15000       Null       16000          18000                15000


-- Lead - Returns the value from Specified Number of Rows after Current Row
-- Lag - Returns the value from Specified Number of Rows before Current Row
-- First Value- Returns the value from first row in current window frame
-- Last Value- Returns the value from Last row in current window frame

-- Note - Value & Aggragate function - Name of C-- oloumn need to add in Bracket
--        Ranking Fuction Bracket need to empty.

-- .Find each employee's salary and rank them within their department */

Select Department_Id,concat(First_Name,' ',Last_name) as Name,Salary,
rank() over
...
(partition by department_id order by salary desc ) as cummulative_salary from employees;
...
(PARTITION BY DEPARTMENT_ID ORDER BY SALARY DESC) AS RNK FROM EMPLOYEES) RANKED WHERE RNK <= 3 ;

# Q5. SHOW EACH EMPLOYEE'S SALARY DIFFERENCE FROM THE DEPARTMENT'S HIGHEST SALARY.
 
SELECT DEPARTMENT_ID, FIRST_NAME, LAST_NAME, SALARY, MAX(SALARY) OVER 
(PARTITION BY DEPARTMENT_ID ORDER BY SALARY DESC) - SALARY AS DIFF_FROM_MAX FROM EMPLOYEES;

-- DATABASE
-- RELATIONAL DATABASE VS NON_RELATIONAL DATABASE
-- DIFFERENCE BETWEEN DDL AND DML COMMANDS
-- SCHEMA
-- ENTITY RELATIONSHIP DIAGRAM
-- DIFFERENCE BETWEEN ALTER AND UPDATE
-- DIFFERENCE DROP,TRUNCATE AND DELETE
-- WHAT IS THE DIFFERENCE BETWEEN ORDER OF EXECUTION AND 
-- CODING ORDER
-- DIFFERENCE BETWEEN WHERE AND HAVING CLAUSE.
-- GROUPING SORTING FILTERING.
-- OPERATORS = IN , BETWEEN , LIKE 
-- CLAUSES= ORDER BY , GROUP BY , HAVING , WHERE , FROM
-- JOINS
-- WINDOWS FUNCTION:-
